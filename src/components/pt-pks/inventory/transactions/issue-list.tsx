...
export function IssueList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async (): Promise<PaginatedResult<GoodsIssueDTO>> => {
      ...
      const res = await fetch(`/api/inventory/issue?${params.toString()}`);
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle>Goods Issue (Pengeluaran Barang)</CardTitle>
              ...
              <TableBody>
                {data?.data.map((issue) => (
                  <TableRow key={issue.id}>
                    ...
                    <TableCell>{format(new Date(issue.date), "dd MMM yyyy")}</TableCell>
                    <TableCell>{issue.warehouse?.name ?? "-"}</TableCell>
                    <TableCell>{issue.costCenter ?? "-"}</TableCell>
                    <TableCell>{issue.items?.length ?? 0}</TableCell>
                    <TableCell className="text-muted-foreground">{issue.note ?? "-"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
