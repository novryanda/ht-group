import { z } from "zod";
import { KaryawanService } from "~/server/services/pt-pks/karyawan.service";
import {
  EmployeeListQuerySchema,
  EmployeeCreateSchema,
  EmployeeUpdateSchema,
  EmployeeFamilyCreateSchema,
} from "~/server/schemas/pt-pks/karyawan";
import type {
  EmployeeListItemDTO,
  EmployeeFamilyDTO,
  EmployeeDTO,
  ...
  static async listKaryawan(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        console.error("❌ Zod validation error in listKaryawan:", error.issues);
        return {
          success: false,
          error: "Invalid query parameters: " + JSON.stringify(error.issues),
          statusCode: 400,
        };
      }
