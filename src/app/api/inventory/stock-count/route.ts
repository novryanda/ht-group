...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.getCountList(query);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/stock-count error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.createCount(body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
