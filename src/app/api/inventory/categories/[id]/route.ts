...
export async function GET(
  ...
  try {
    ...
    const result = await InventoryAPI.getCategoryById(id);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/categories/[id] error:", error);
...
export async function PATCH(
  ...
  try {
    ...
    const result = await InventoryAPI.updateCategory(id, body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    console.error("DELETE /api/inventory/categories/[id] error:", error);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
