...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await InventoryAPI.getGrnList(query);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/grn error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await InventoryAPI.createGrn(body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("POST /api/inventory/grn error:", error);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
