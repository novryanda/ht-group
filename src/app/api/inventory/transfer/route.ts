...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.getTransferList(query);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/transfer error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.createTransfer(body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
