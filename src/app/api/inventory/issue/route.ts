...
export async function GET(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.getIssueList(query);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/issue error:", error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await InventoryDocsAPI.createIssue(body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("POST /api/inventory/issue error:", error);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
