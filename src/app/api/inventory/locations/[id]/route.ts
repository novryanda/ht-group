...
export async function GET(
  ...
  try {
    ...
    const result = await InventoryAPI.getLocationById(id);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("GET /api/inventory/locations/[id] error:", error);
...
export async function PATCH(
  ...
  try {
    ...
    const result = await InventoryAPI.updateLocation(id, body);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    const { id } = await params;
    const result = await InventoryAPI.deleteLocation(id);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("DELETE /api/inventory/locations/[id] error:", error);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
