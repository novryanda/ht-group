...
/**
 * GET /api/pt-pks/divisi
 * Get list of divisi with pagination
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await DivisiAPI.getList(query, companyId);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("❌ GET /api/pt-pks/divisi error:", error);
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await DivisiAPI.create(body, companyId);
    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("❌ POST /api/pt-pks/divisi error:", error);
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Internal server error",
