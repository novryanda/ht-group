...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check role - only PT_PKS_ADMIN, EXECUTIVE, GROUP_VIEWER can access
    ...
    const result = await BuyerAPI.getBuyers(queryParams);
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (userRole !== "PT_PKS_ADMIN") {
      return NextResponse.json(
        { success: false, error: "Forbidden - Only PT_PKS_ADMIN can create buyers" },
        { status: 403 }
      );
    }
    ...
    // Create buyer
    const result = await BuyerAPI.createBuyer(body, currentUserId);
