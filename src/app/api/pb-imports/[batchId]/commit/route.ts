...
import { commitBatchSchema } from "~/server/schemas/pb-import";
...
export async function POST(
  ...
  try {
    ...
    // Role check - only PT_PKS_ADMIN and EXECUTIVE can commit
    ...
    const validated = commitBatchSchema.parse(body);

    // Call application API
    const result = await PbImportAPI.commitBatch(batchId, validated);

    return NextResponse.json(result, { status: result.statusCode });
  } catch (error) {
    console.error("POST /api/pb-imports/[batchId]/commit error:", error);
