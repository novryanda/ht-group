...
export const PT_COMPANIES: PTCompany[] = [
  {
    ...
    dashboardPath: "/pt-nilo/dashboard",
  },
  {
    code: "PT-ZTA",
    name: "PT ZTA",
    adminRole: "PT_ZTA_ADMIN",
    dashboardPath: "/pt-zta/dashboard",
  },
  {
    code: "PT-TAM",
    name: "PT TAM",
    adminRole: "PT_TAM_ADMIN",
    dashboardPath: "/pt-tam/dashboard",
  },
  {
    code: "PT-HTK",
    name: "PT HTK",
    adminRole: "PT_HTK_ADMIN",
    dashboardPath: "/pt-htk/dashboard",
  },
];
...
export function canAccessRoute(session: Session | null, route: string): boolean {
  ...
  // Check if route is PT-specific
  for (const pt of PT_COMPANIES) {
    if (route.startsWith(`/${pt.code.toLowerCase()}`)) {
...
export function getCompanyCodeFromRoute(route: string): string | null {
  for (const pt of PT_COMPANIES) {
    const ptPath = `/${pt.code.toLowerCase()}`;
